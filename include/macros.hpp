...
#define AMT_UNROLL_LOOP8_IMPL(OFFSET,...)   \
    AMT_UNROLL_STATEMENT((OFFSET + 0),__VA_ARGS__)     \
    AMT_UNROLL_STATEMENT((OFFSET + 1),__VA_ARGS__)     \
    AMT_UNROLL_STATEMENT((OFFSET + 2),__VA_ARGS__)     \
    AMT_UNROLL_STATEMENT((OFFSET + 3),__VA_ARGS__)     \
    AMT_UNROLL_STATEMENT((OFFSET + 4),__VA_ARGS__)     \
    AMT_UNROLL_STATEMENT((OFFSET + 5),__VA_ARGS__)     \
    AMT_UNROLL_STATEMENT((OFFSET + 6),__VA_ARGS__)     \
    AMT_UNROLL_STATEMENT((OFFSET + 7),__VA_ARGS__)     
    // C[I] += A[I + W * (7+J)] * b[7 + J];
