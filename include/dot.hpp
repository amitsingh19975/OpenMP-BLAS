...
namespace amt {
    ...
    void dot_prod_helper_same_layout_ref(
        Out* c,
        In const* a,
        In const* b, 
    ...
    ) noexcept
    {
        ...
        // #pragma omp parallel
        {
            #pragma omp for simd safelen(16) schedule(simd:static,64) aligned(a,b:alignment)
            for(auto i = 0ul; i < n; ++i){
                sum += (a[i] * b[i * wb]);
            }
        }
    ...
    )
    {
        ...
        if constexpr( std::is_same_v<layout_type1,layout_type2> ){
            dot_prod_helper_same_layout_ref(
                &c,
                a.data(),
                b.data(),
                1ul,
            ...
            dot_prod_helper_diff_layout_ref(
                &c,
                b.data(),
                a.data(),
                WA,
